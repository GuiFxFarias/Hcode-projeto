...
if (page) {
  ...
  const tiposHamburgueresEl = page.querySelector(
    ".category.hamburguer ul"
  ) as HTMLElement;
  const tiposExtrasEl = page.querySelector(
    ...
    ingredientEls.forEach((ingredient) => {
      if (ingredient.querySelector("input:checked")) {
        ...
        if (ingredientName != null && !isNaN(ingredientPrice)) {
          ...
          console.log(
            "Erro: esse ingrediente não possuía descrição ou preço válido"
          );
        }
      }
    });
